...
import com.example.property.management.utils.IPUtil;
    ...
    public ApiResponse beforeBodyWrite(ApiResponse apiResponse, MethodParameter methodParameter, MediaType mediaType, Class<? extends HttpMessageConverter<?>> aClass, ServerHttpRequest serverHttpRequest, ServerHttpResponse serverHttpResponse) {
        if (apiResponse != null) {
            ...
            if (apiResponse.getSuccess().equals(true)) {
                ...
                log.info("\nrequestURL: {},\nheaders:{},  \nuserIP: {}, \ndata: {}", request.getRequestURL(), getRequestHeaders(request), IPUtil.getUserIP(request), apiResponse.getData());
            }
            if (apiResponse.getSuccess().equals(false)) {
                log.error("\nException Occurred! \nrequestURL: {},\nheaders:{}, \nuserIP: {}, \nerrorMessage: {}", request.getRequestURL(), getRequestHeaders(request), IPUtil.getUserIP(request), apiResponse.getMessage());
            }
        }
        return apiResponse;
    }
